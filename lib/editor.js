...
const NUMBER_REGEXP = /#[0-9a-f]{3,6}|(\-?\d*\.?\d+)/g
...
export default class Editor {
  constructor(textEditor) {
    ...
    this.tokens = []
    this.markers = {}
    this._tokenize = debounce(this.tokenize, 60)
  }
  activate() {
  ...
  tokenize() {
    ...
    const editorText = editorBuffer.getText()
    let match = null
    do {
      match = NUMBER_REGEXP.exec(editorText)
      if (match && match[1]) {
        const range = Range.fromObject([
          editorBuffer.positionForCharacterIndex(match.index),
          editorBuffer.positionForCharacterIndex(match.index + match[1].length)
        ])
        const key = rangeKey(range)
        const tokenInfo = {key, range, pointer: null, marker: null, text: match[1]}
